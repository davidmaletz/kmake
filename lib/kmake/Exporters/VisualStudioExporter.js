...
class VisualStudioExporter extends Exporter_1.Exporter {
    ...
    async exportProject(from, to, project, platform, cmd, noshaders, options) {
        ...
        if (project.livePP) {
            ...
            debugDefines += 'KINC_LIVEPP;KINC_LIVEPP_PATH=L"' + liveppPath.replace(/\\/g, '\\\\') + '";';
        }
        let incstring = '';
        ...
        if (platform === Platform_1.Platform.Windows) {
            this.p('<ItemGroup>', 1);
            for (let file of project.getFiles()) {
                if (Project_1.Project.kincDir && Project_1.Project.kincDir.toString() !== '' && !noshaders && file.file.endsWith('.glsl')) {
